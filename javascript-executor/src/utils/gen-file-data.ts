...
export const genFileData = async (id: string): Promise<string> => {
  const separator = '|';
  const path = `${cwd()}/dist/temp/${new Date().getDay()}/${id}`;
  ...
  await writeFile(`${path}/${solutionTxt}`, fakeSolution);
  await writeFile(`${path}/${funcNameTxt}`, fakeFuncName);
